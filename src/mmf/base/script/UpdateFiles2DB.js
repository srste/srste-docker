...
module.exports = function (files2Update, app, lastSyncId, ws) {
    ...
    return new Promise((resolve, reject) => {
        ...
        .then(function(localSyncConfigMap) {
            ...
            if(validPatterns2Process && validPatterns2Process.length > 1) {
                ...
                if(patternsBasedOnName) {
                    ...
                    for(var categoryNameIndx in categoryNames) {
                        let categoryName = categoryNames[categoryNameIndx];                    
        ...
        .then(async function(patterns2Execute) {         
            ws.send(JSON.stringify({type: srsteeConstants.pushStatusHeader, payload: {total: patterns2Execute.length, processed: 0}}));   
            if(patterns2Execute && patterns2Execute.length > 0){
                ...
                for(var indx=0; indx<orders.length; indx++) {
                    ...
                    processedItems = processedItems + (patternsAtSameLevel[orders[indx]]?patternsAtSameLevel[orders[indx]].length: 0);
                    ws.send(JSON.stringify({type: srsteeConstants.pushStatusHeader, payload: {total: patterns2Execute.length, processed: processedItems}}));   
                }
                resolve();
            }else {
                resolve();
...
function executePatterns(patterns2Execute, $tbl, $qry, $script) {
    return new Promise(async (resolve, reject) => {
        if(patterns2Execute && patterns2Execute.length > 0){
            ...
            for(var patternIndx2Execute in patterns2Execute) {      
                try {                                             
                    let pattern2Execute = patterns2Execute[patternIndx2Execute];                    
                    let evalValue = await evaluateCondition(pattern2Execute.fileContent, $qry, $script, pattern2Execute.importCondition)                
                    if(evalValue == true) {
                        try{
                            await executeScript(pattern2Execute.script, pattern2Execute.table, pattern2Execute.input, 
        ...
        try{
            if(script) {
                ...
                try {
                    ...
                    console.error('Error occurred while loading the script - ' + script2ExecuteStr);
                    resolve({});
                }                                                
            }else{
                resolve({});
