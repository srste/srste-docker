...
*/
var factoryFn = function($, GridStackUI, scope) {
  /**
  ...
  */
  ...
  JQueryUIGridStackDragDropPlugin.prototype.off = function(el, eventName, callback) {
    $(el).off(eventName, callback);
    return this;
  };
...
var factoryFn2 = function($, scope, rootEl, isPreview, dimensions) {
    ...
    var Utils = {
      ...
      createStylesheet: function(id, el) {
      ...
      removeStylesheet: function(id, el) {
        if(el)
          $('STYLE[data-gs-style-id=' + id + ']', el).remove();
        else 
    ...
    */
    ...
    var GridStack = function(el, opts) {
      ...
      if (!self.opts.staticGrid && typeof self.opts.removable === 'string') {
        ...
        this.dd
          .off(trashZone, 'dropover', ()=> {})
          .off(trashZone, 'dropout', ()=> {});          
      ...
      if (!self.opts.staticGrid && self.opts.acceptWidgets) {
        ...
        //remove existing drag/drop events on the container element to respect the new event bindings

        this.dd
        .off(self.container, 'dropover', ()=> {})
        .off(self.container, 'dropout', ()=> {})
        .off(self.container, 'drop', ()=> {});
    ...
    GridStack.prototype._initStyles = function() {
      if (this._stylesId) {
        Utils.removeStylesheet(this._stylesId, this.container);
      }
...
var gridStackFn = function(localjQuery, scope, rootEl, isPreview, dimensions) {
  return new Promise((resolve, reject) => {
    var jQueryInstance, scope;    

    if (localjQuery) {
      jQueryInstance = localjQuery;
    }

    if(window && jQueryInstance) {
      resolve(factoryFn2(jQueryInstance, window, rootEl, isPreview, dimensions));
    }else if(window && window.jQuery) {
      resolve(factoryFn2(window.jQuery, window, rootEl, isPreview, dimensions));
    }else if (typeof define === 'function' && define.amd) {
      if (jQueryInstance) {
        define(['exports'], function(scope) {
          resolve(factoryFn2(jQueryInstance, scope, rootEl, isPreview, dimensions));
        });
      } else {
        define(['jquery', 'exports'], function(jQueryInstance, scope) {
          resolve(factoryFn2(jQueryInstance, scope, rootEl, isPreview, dimensions));
        });
      }
      ...
      try {
        jQueryModule = jQueryInstance || require('jquery');
      } catch (e) {}

      resolve(factoryFn2(jQueryModule || window.jQuery, exports, rootEl, isPreview, dimensions));
    } 
  });
};

var gridStackUIFn = function(jQuery, gridStackUI, scope) {

  if(scope && jQuery) {
    return factoryFn(jQuery, gridStackUI, scope);
  }else if(scope && scope.jQuery) {
    return factoryFn(scope.jQuery, gridStackUI, scope);
  }else if (typeof define === 'function' && define.amd) {
    define(['exports', 'jquery-ui/ui/data', 'jquery-ui/ui/disable-selection',
      'jquery-ui/ui/focusable', 'jquery-ui/ui/form', 'jquery-ui/ui/ie', 'jquery-ui/ui/keycode', 'jquery-ui/ui/labels',
      'jquery-ui/ui/jquery-1-7', 'jquery-ui/ui/plugin', 'jquery-ui/ui/safe-active-element', 'jquery-ui/ui/safe-blur',
      'jquery-ui/ui/scroll-parent', 'jquery-ui/ui/tabbable', 'jquery-ui/ui/unique-id', 'jquery-ui/ui/version',
      'jquery-ui/ui/widget', 'jquery-ui/ui/widgets/mouse', 'jquery-ui/ui/widgets/draggable',
      'jquery-ui/ui/widgets/droppable', 'jquery-ui/ui/widgets/resizable'
    ], function(scope) {
      return factoryFn(jQuery, gridStackUI, scope);
    });
  } else if (typeof exports !== 'undefined') {
    ...
    try {
      GridStackUI = gridStackUI || require('gridstack').GridStackUI;
    } catch (e) {}
    return factoryFn(jQuery, gridStackUI, exports);
  }
};

// module.exports.gridStackFn = gridStackFn;
// module.exports.gridStackUIFn = gridStackUIFn;

export {gridStackFn, gridStackUIFn}
